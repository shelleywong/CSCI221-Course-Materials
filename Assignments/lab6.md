...
As usual, this is an individual project. All hand-ins are electronic. Clarifications and corrections will be posted in this Git repo or on Canvas.

## Handout Instructions

Start by downloading `perflab-setup.tar` from Canvas and copying it to the directory in which you plan to do your work. For this assignment, you may use a (protected) directory on ecc-linux or another Linux x86_64 machine in which you plan to do your work.

Extract the files from the provided tar file:

```bash
  $ tar -xvf perflab-setup.tar
```

This will cause a number of files to be unpacked in a `perflab-setup` directory. These are the files that you may modify and will need to submit:
- FilterMain.cpp
- Filter.cpp
- Filter.h
- Makefile
...
The “filter” is an *n x n* array of numbers. We’ll go through the logistics of how a “filter” works in class and briefly summarize it here. Basically, you can cause a number of visual affects by applying a filter to an image. The filter is implemented as a “convolution”, which means that elements of the filter matrix are multiplied by the image matrix to compute a new value for the image. Pictorially, this is represented as:

![Figure 1: Filter Operation](/Assignments/Images/PerfLabFilterOp.png)
Figure 1: Filter Operation

Computationally, this is structured as five nested `for` loops (three to go over the colors, rows, and columns and two more to apply the filter). In the solution provided to you, filters are represented using the `Filter` class, implemented in *Filter.h* and *Filter.cpp*.
...
The Makefile also provides a `make judge` rule that runs the test images and filters. You will want to comment out the `judge` goals prior to submitting your code to INGInious. Otherwise, `make judge` will run as part of the compilation process, and it will take significantly longer to run. In Makefiles, the `#` symbol indicates a comment:
...
Different computers will run this code at different speeds (even measured in “cycles per second”). In order to have a level playing field, everyone must submit their code to INGInious, which will use the *csciperfproject* system to time project submissions. For this lab, everything is set up to ensure that you are the only person running your code on the *perf* machines at a given time, ensuring a consistent result.

You may do your development for this lab in the x86-64 environment of your choice (e.g. ecc-linux, WSL2, or x86-64 virtual machine). Before submitting to INGInious, you should confirm your code compiles and runs successfully. The *perf* machines should be used to validate your measurements or improvements.

Download the code from Canvas right away and make certain you can compile and run the distributed version and achieve times comparable to those above.

You should also make use of *git* version control so you can roll back if the photos stop filtering correctly and so you can commit every change and your justification for why it makes your code run faster, which will be helpful for explaining your code in the in-class quiz about the Performance Lab.

## Submitting your Assignment

You should upload the following files to INGInious by the due date:
...
You are not required to make changes to all of these files, but these are the files that you may modify for this assignment. In FilterMain.cpp, do NOT modify or change the location of any `#if defined` code blocks.

Because the Makefile does not have an extension, you will need to copy/paste your Makefile code into a code form field. If you receive a compiler error that includes the following message, "*** missing separator. Stop.", then you are likely using spaces instead of tabs to indent the makefile recipe lines. Remove any spaces used for indentation and replace them with tabs, then try submitting your code again.

You need to be able to explain *why* your code modifications improve the running time for the program and explain what would happen with minor modifications. Again, you must be able to explain *why* you got the performance you did, so you should take notes for why you made each modification so you have a reference when you are asked to explain this as part of the in-class Performance Lab quiz (worth 10 points). If you do not submit anything for the Performance Lab, you will not receive credit for the in-class quiz.
